...
class Policy(nn.Module):
    ...
    def forward_step(self, camera_obs, proprio_obs, lstm_state):
        # Apply convolutional layers to the camera observation
        ...
        # Flatten the visual encoding tensor
        vis_encoding = torch.flatten(vis_encoding, start_dim=1)
        # Concatenate visual encoding and proprioceptive observations
        ...
        lstm_state = (h, c)
        # Apply a tanh activation to the output of the LSTM
        out = torch.tanh(self.linear_out(lstm_out))
        # Return the output and the updated LSTM state
