...
if current_step == 1:
    ...
    # Call Action LMP: composer("move to block3")
    block3 = get_object('block3')
    gripper = get_object('gripper')
    action = block3.position - gripper.position
    move_gripper(action)
    # action done
    # Call Check LMP: composer("move to block3")
    block3 = get_object('block3')
    gripper = get_object('gripper')
    if np.linalg.norm(gripper.position - block3.position) < 0.02:
        increment_current_step()
    # done check
    ...
    # Call Action LMP: composer("move 5cm above the block2 on target")
    block2 = get_object('block2')
    gripper = get_object('gripper')
    goal_position = block2.position + np.array([0, 0, 0.05])
    action = goal_position - gripper.position
    move_gripper(action)
    # action done
    # Call Check LMP: composer("move 5cm above the block2 on target")
    block2 = get_object('block2')
    gripper = get_object('gripper')
    goal_position = block2.position + np.array([0, 0, 0.05])
    if np.linalg.norm(gripper.position - goal_position) < 0.02:
